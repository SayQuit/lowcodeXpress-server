...
detailRouter.post('/', async (req, res) => {
    ...
    try {
        ...
        const { element, name, description, type, tech, variable, event, props, createAt } = detailRow
        sendData(res, { element, name, description, type, tech, variable, event, props, createAt })
    } catch (error) {
        sendFail(res)
    }
